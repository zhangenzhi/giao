...
def _opt_train_step(unet, regs, train_inputs, train_labels, labels):
    ...
    with tf.GradientTape() as tape:
        ...
        for idx in range(len(regs)):
            ...
            l1_loss = tf.reduce_mean(tf.abs(pseudo_inputs - train_inputs)) * 96
            phase_loss = tf.reduce_mean(tf.square(pseudo_latent_phase - train_latent_phase)) * 1024
...
def giao(unet, model,  model_args, dataloader, iter_train, test_data):
    ...
    for t in range(100):
        ...
        for j in range(TRAIN_STEP):
            ...
            if j % 100 == 0:
                print("Epoch:{} GIAO Train Loss:{}, L1 Loss:{}, Phase Loss:{}".format(j, giao_train_loss, l1_loss, phase_loss))
